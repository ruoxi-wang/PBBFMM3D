...
Ns = 5e3; % Number of sources in simulation cell
Nf = 5e3; % Number of field points in simulation cell
L = 1;    % Length of simulation cell (assumed to be a cube)
dof_s = 3;% Dimension of the tensor kernel
dof_f = 3;
...
fid = fopen('source1e3_test.bin', 'w');
fwrite(fid, source_binary, 'double');
fclose(fid);

fid = fopen('field1e3_test.bin', 'w');
fwrite(fid, field_binary, 'double');
fclose(fid);

fid = fopen('charge1e3_test.bin', 'w');
fwrite(fid, q, 'double');
fclose(fid);
