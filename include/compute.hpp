...
public:
    ...
    void FMMCompute(nodeT **A, vector3 *field, vector3 *source, double *charge,
                    ...
                    doft *cutoff, int n, doft *dof,double*stress);
    void UpwardPass(nodeT **A, vector3 *source, double *q, double *Cweights, double *Tkz, double *VT, double *Kweights, doft *cutoff, int n, doft *dof);
    void FMMInteraction(nodeT **A, double *E, int *Ktable, double *U,
                        double *VT, double *Kweights, int n, doft *dof,
                        doft *cutoff, double homogen, int curTreeLevel);
    void DownwardPass(nodeT **A, vector3 *field, vector3 *source,
                      double *Cweights, double *Tkz, double *q, doft *cutoff,
                      int n, doft *dof, double *stress);
    void EvaluateField(vector3 *field, vector3 *source, double *q, int Nf,
                       int Ns, doft *dof, double *fieldval);
    void Local2Local(int n, double *r, double *F, double *Fchild, doft *dof, double *Cweights);
...
H2_3D_Compute<T>::H2_3D_Compute(T * FMMTree,vector3 * field, vector3 *source, int Ns, int Nf, double* charge, int m, double* stress) {
    ...
    for (int i = 0; i < m; i++) {
        FMMCompute(&FMMTree->tree,field,source,&charge[i*Ns*FMMTree->dof->s],FMMTree->K,FMMTree->U,FMMTree->VT,FMMTree->Tkz,FMMTree->Ktable,FMMTree->Kweights,FMMTree->Cweights,
                   FMMTree->homogen,&FMMTree->cutoff,FMMTree->n,FMMTree->dof,&stress[i*Nf*FMMTree->dof->f]);
    }
...
template <typename T>
void H2_3D_Compute<T>::FMMCompute(nodeT **A, vector3 *field, vector3 *source, double *charge,
                                  double *K, double *U, double *VT, double *Tkz, int *Ktable,
                                  double *Kweights, double *Cweights, double homogen,
                                  doft *cutoff, int n, doft *dof,
                                  double *stress) {
    
	UpwardPass(A,source,charge,Cweights,Tkz,VT,Kweights,cutoff,n,dof);
    // Computes all of the cell interactions
	FMMInteraction(A,K,Ktable,U,VT,Kweights,n,dof,cutoff,homogen,0);
    DownwardPass(A,field,source,Cweights,Tkz,charge,cutoff,n,dof,stress);
}

template <typename T>
void H2_3D_Compute<T>::UpwardPass(nodeT **A, vector3 *source, double *q, double *Cweights, double *Tkz, double *VT, double *Kweights, doft *cutoff, int n, doft *dof) {
	...
	if ((*A)->leaves[0] != NULL) {  // Going up the tree M2M
        ...
        FMMTree->ComputeSn(sourcet,Tkz,n,Ns,Ss);
...
void H2_3D_Compute<T>::FMMInteraction(nodeT **A, double *E, int *Ktable, double *U,
    ...
    // Initialize all proxy field values to zero
    Pf = (double *)calloc(cutoff_f, sizeof(double));
    ...
    for (i=0;i<ninter;i++) {
        ...
        //B->proxysval = (double *)malloc(cutoff_s * sizeof(double));
        double FFCoeff[cutoff_f];
        Moment2Local(n, R, B->sourceval, FFCoeff, E +Ksize*shift, Ktable,
                     dof, cutoff, VT +Vsize*shift, Kweights);
    ...
    dgemv_(trans, &dofn3_f, &cutoff_f, &scale, U +Usize*shift, &dofn3_f, Pf, &incr,
           &beta, F, &incr);
    // 3c
	...
	if ((*A)->leaves[0] != NULL) {
	    ...
	    for (i=0;i<8;i++) {
            if ((*A)->leaves[i]->Nf != 0) {
                ...
                Local2Local(n, r, F, Fchild, dof, Cweights);
        ...
        *sourcepos;
        ...
        FMMTree->ComputeSn(fieldt,Tkz,n,Nf,Sf);
 ...
 */
template <typename T>
void H2_3D_Compute<T>::Local2Local(int n, double *r, double *F, double *Fchild, doft *dof, double *Cweights){
